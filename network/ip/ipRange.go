...
func (b *IPRange) Start2IP() string {
	return utils.Long2IP(b.Start)
}
func (b *IPRange) End2IP() string {
	return utils.Long2IP(b.End)
}
...
func (b *IPRange) GetAllIPToStr() []string {
	ret := make([]string, 0)
	b.Do(func(u uint32) bool {
		ret = append(ret, utils.Long2IP(u))
		return true
	})
	return ret
}
...
func ipStrToIPRange(ipStr string) (*IPRange, error) {
	...
	if gstr.Contains(ipStr, "/") {
		...
		end = utils.IP2long(strings.Join(tSIP, "."))
		//} else {
		//	end = Ip2long(eIP)
		//
		//}
		...
		start = utils.IP2long(strings.Join(sIP, "."))
		end = utils.IP2long(strings.Join(eIP, "."))

	} else {
		start = utils.IP2long(ipStr)
		end = start + 1
	}
	if start > end {
